...
// Tally the number of NaN and assign an alert level to a data sample.
//
// Sample input data:
...
string handle(string req) {
    auto data_json = nlohmann::json::parse(req);
    ...
    level -= simulate_compute(data, sum, count);
    ...
    try {
        ...
        // Concat SQL string
        string query1 = "SELECT * FROM nan_count ORDER BY arrived_at DESC LIMIT 2;";

        // execute the first query
        res = stmt -> executeQuery(query1);

        vector<int> recent_pass, recent_nan_c, recent_level;
        while (res->next()) {
            recent_pass.push_back(res->getInt("pass"));
            recent_nan_c.push_back(res->getInt("count"));
            recent_level.push_back(res->getInt("alert_level"));
        }

        recent_pass.push_back(pass);
        recent_nan_c.push_back(nan_count);
        int recent_h_nan_counts = 0, recent_pass_counts = 0;
        for(vector<int>::iterator it = recent_pass.begin(); it != recent_pass.end(); it++){
            if(*it == 1)
                recent_pass_counts += 1;
        }
        for(vector<int>::iterator it = recent_nan_c.begin(); it != recent_nan_c.end(); it++){
            if(*it > 8)
                recent_h_nan_counts += 1;
        }
        if(recent_pass_counts > 1 || recent_h_nan_counts > 1)
            level -= 1;

        string query2 = "INSERT INTO nan_count(pass, arrived_at, count, alert_level) VALUES(" + to_string(pass) + ", \"" + format_date + "\", " + to_string(nan_count) + ", " + to_string(level) + ");";
        stmt = con -> createStatement();
        stmt -> execute(query2);

        output["time"] = format_date;
        output["pass"] = pass;
        output["nan_count"] = nan_count;
        output["alert_level"] = level;
